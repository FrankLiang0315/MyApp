    ...
    public class AuthenticateController : InfoController
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest request)
        {
            ...
            if (userExists != null)
                ...
                {
                    Status = "Error", ErrorCode = Error.Code.UserAlreadyExists,
                    Message = Error.GetMessage(Error.Code.UserAlreadyExists)
                });
            ...
            if (request.Password != request.ConfirmPassword)
            {
                return Ok(
                    new Response
                    {
                        Status = "Error", ErrorCode = Error.Code.PasswordNotMatch,
                        Message = Error.GetMessage(Error.Code.PasswordNotMatch)
                    });
            }
            ...
            if (!result.Succeeded)
                return Ok(new Response
                {
                    Status = "Error", ErrorCode = Error.Code.UserCreateFailed,
                    Message = Error.GetMessage(Error.Code.UserCreateFailed)
                });
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordRequest model)
        {
            ...
            if (user == null)
            {
                ...
                return Ok(new Response
                {
                    Status = "Error", ErrorCode = Error.Code.UserNotExists,
                    Message = Error.GetMessage(Error.Code.UserNotExists)
                });
            }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordRequest model)
        {
            if (ModelState.IsValid)
            {
                ...
                // 密碼重設失敗，可能顯示錯誤信息
                MyLog.Info("error", result.Errors.FirstOrDefault().ToJson());
                return Ok(new Response
                {
                    Status = "Error", ErrorCode = Error.Code.ResetPasswordFailed,
                    Message = Error.GetMessage(Error.Code.ResetPasswordFailed)
                });
            }
